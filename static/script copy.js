// script.js
// Purpose: 
//  1. Build the JSON payload from the form
//  2. Send it to the backend /predict endpoint,
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener('submit', function (event) {
        ...
        // NOTE: keep data construction and types here in one place; if you change the API payload shape,
        // update both places in this file (see duplicated fetch in validateForm()).
        const data = {
            // Features
            'features': {
        ...
        // NOTE: This fetch (in the submit handler) posts to /predict — there is a second, identical fetch
        // inside validateForm() further down. Both fetches perform the same POST; if you want to point
        // to a different backend for development you must change the URL used by both calls or centralize
        // the base URL into a single constant/function.
...
function validateForm() {
    ...
    const ageValue = ageSelect.value === 'under_18' ? 18 :
        ageSelect.value === '18_24' ? 21 :
            ageSelect.value === '25_34' ? 30 :
                ageSelect.value === '35_44' ? 40 :
                    ageSelect.value === '45_54' ? 50 :
                        ageSelect.value === '55_64' ? 60 : 75;

    const data = {
        student_name: formData.get('student-name'),
        student_email: formData.get('student-email'),
        features: {
            age: parseFloat(ageValue), // float32
            dem_female: parseInt(formData.get('gender') === 'female' ? 1 : 0, 10), // int8
            race: parseInt(formData.get('race') === 'Black' ? 1 : 0, 10), // int8
    ...
    // Send the data to the API via fetch. Replace with Azure endpoint when deploying.
    // NOTE: This is the second fetch in this file (duplicated behavior). If you change the URL above but not here,
    // you will have inconsistent behavior depending on which function is executed. Both must use the same base URL
    // when you switch environments, or you should centralize the URL into a single place.
    fetch('https://ethicalhealthscore01-fgh3fcdhh2fuazd4.centralus-01.azurewebsites.net/predict', {
        // fetch('http://127.0.0.1:5000/predict', {
        method: 'POST',
        headers: {
